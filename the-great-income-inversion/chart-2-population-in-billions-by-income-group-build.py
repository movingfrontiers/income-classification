...
share={(y,g): sum(pop_at(nm,y) for nm in names if cls_at(nm,y)==g)/1000. for y in STAGES for g in order}
flows={}
for k in range(len(STAGES)-1):
    ...
    for nm in names:
        ...
        m[(g0,g1)][0]+=pop_at(nm,t0)/1000.
        m[(g0,g1)][1]+=pop_at(nm,t1)/1000.
    flows[k]=m
GAPV=0.16; NODE_W=3.0
XPOS={y:i*8 for i,y in enumerate(STAGES)}
def stack_pos(y):
    pos={}; cur=sum(share[(y,g)] for g in order)+3*GAPV
    for g in order:
        pos[g]=(cur-share[(y,g)],cur); cur-=share[(y,g)]+GAPV
    return pos
POS={y:stack_pos(y) for y in STAGES}
TOP=max(sum(share[(y,g)] for g in order)+3*GAPV for y in STAGES)
def bez(p0,p1,s):
...
for y in STAGES:
    for g in order:
        ...
        ax.text(XPOS[y],(b+t)/2,f'{share[(y,g)]:.1f}',ha='center',va='center',
                color='white',fontsize=9,fontweight='bold',zorder=6)
    ax.text(XPOS[y],TOP+0.32,str(y),ha='center',fontsize=12.5,fontweight='bold',color='#444')
def halo(x,y,txt):
...
if a4: halo(a4[0],a4[1]-0.185,'INDIA + 26 \u2192 LM')
if a1: halo(a1[0],a1[1]-0.185,'CHINA + 25 \u2192 UM')
if a2: halo(a2[0],a2[1]-0.185,'CHINA + 19 \u2192 HIGH')
if a3: halo(a3[0],a3[1]-0.37,'INDIA + 8 \u2192 UM')
ax.text(XPOS[2030]+0.5,-0.385,'projection from mid-2020s \u2192',fontsize=10.5,style='italic',color='#555')
ax.set_xlim(-4.9,50.6); ax.set_ylim(-0.56,TOP+0.46)
ax.axis('off')
...
fig.savefig(OUT+'chart-2-population-in-billions-by-income-group.png',dpi=200); plt.close(fig)
print('c14u saved')
...
subtitle = "Population in billions by country income group"
im=Image.open(OUT+'chart-2-population-in-billions-by-income-group.png').convert('RGB'); W,H=im.size
...
canvas.save(OUT+'chart-2-population-in-billions-by-income-group.png',optimize=True)
print('titled chart-2-population-in-billions-by-income-group.png  %dx%d'%canvas.size)
